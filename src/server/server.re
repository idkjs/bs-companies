...
let employeesToJson = (employees: list(employee)) =>
  Array.of_list(employees) |> Array.map((e) => Js_json.object_(employeeToJson(e)));
...
let getBody = ((body, key)) =>
  switch body {
  | Some(json) => getDictString(json, key)
  | _ => None
  };

App.get(app, ~path="/companies") @@
Middleware.from(
  (_req, res, _next) => {
    let jsonCompanies = companies^ |> Array.of_list |> Array.map((c) => Js.Json.string(c.name));
    setRespHeaders(res);
    Response_.sendJson(res, Js_json.array(jsonCompanies))
  }
);

App.get(app, ~path="/employees") @@
Middleware.from(
  (req, res, _) => {
  ...
  (req, res, _) => {
    ...
    let names =
      ...
      |> List.map(
           (c) =>
             c.employees
             |> List.map((e) => c.name ++ (" " ++ (e.firstName ++ (" " ++ e.lastName))))
         )
      |> List.flatten;
...
Middleware.from(
  (req, res, _) => {
            ...
            c.name == name ?
              {
                ...
                employees:
                  company.employees
                  @ [{firstName, lastName, birthday, salary: float_of_string(salary)}]
              } :
              c,
          companies^
        );
    ...
    | EmployeeFieldMissig =>
      Response.sendJson(res, makeFailureJson("One of required employee fields is missing"))
    }
  }
);
